...
#print(doc)
doc = (doc.split(','))
mainpath=r"C:\\Users\\user\\rajagiri"
...
database = MySQLdb.connect(host="localhost", user="root", passwd="nirmalsql", db="rajagirihospital1")
...
wb = xlrd.open_workbook("C:\\python\\Python38\\extract.xlsx")
...
stime=[]
timel=[]
dater=re.compile("([0]?[1-9]|[1|2][0-9]|[3][0|1])[./-]([0]?[1-9]|[1][0-2])[./-]([0-9]{4}|[0-9]{2})")
...
if(output!=[]):
    ...
    if flag:
        ...
        for i in dict2.keys():
            ...
            n += 1
        #print(df)
        file = open("C:\\python\\Python38\\data_p", "wb")
        pk.dump(df, file)
    else:
    ...
    tp= xlrd.open_workbook("C:\\python\\Python38\\result.xlsx")
    ...
    lo = "C:\\python\\Python38\\ReferenceRange.xlsx" # reference excel location
    ...
    for j in range(1,nrtp) :
        ...
        for i in range(1,nro):
            if(sheet.cell_value(i, 0).lower()==tname[j].lower()): 
                if(similar(sheet.cell_value(i, 1).lower(),pname[j].lower())>0.7):  #check if similarity % > 70%
                    if(int(sheet.cell_value(i,4))==0 and int(sheet.cell_value(i,5))==0):
                        if(str(sheet.cell_value(i,6))=="Not specified"):
                            ...
                            if(float(reval[j])>=float(v1) and float(reval[j])<=float(v2)): #normal or abnormal check
                                norm="Normal"
                            else:
                                norm="Abnormal"
                            vx=" "       
...
for r in range(1, sheet.nrows):
    ...
    values = (Patient_ID, Test_Parameter, Values, Unit, Date, Time, Test_Name, Original_Range, Conditions, Remarks, Path)
...
import shutil
src = "C:\\python\\Python38\\result.xlsx"
#src = src + excelsheet
dest = "C:\\Users\\user\\Group2"
shutil.copy(src, dest)

src1 = "C:\\Users\\user\\Group2\\result.xlsx"
desc = "C:\\Users\\user\\Group2\\"
